...
- **Google Workspace:** Onboarded 2026-03-21. Gmail, Calendar, Drive, Sheets, Docs, Tasks, Contacts enabled.
  ...
  - Note: bootstrap TOOLS.md says "No Google accounts configured" — this is a 53-day-stale file. Google IS set up. Reconnect in AlphaClaw UI to regenerate.
...
**Instance:** Josh — Heather Schwartz (personal assistant)  
**New since yesterday:** oc-path plugin bundled; iMessage now 16 days silent; all 11 prior recommendations remain unimplemented after 24 days.

---

## Status Review — Day 24

All 11 prior recommendations remain unimplemented.

### oc-path Plugin — Addendum to Recommendation 1 (HEARTBEAT.md)

Once the oc-path plugin is enabled post-update, HEARTBEAT.md tasks can reference workspace files via portable `oc://` URLs. Add to the HEARTBEAT.md template (Recommendation 1):

```markdown
## Workspace References
- Use oc://workspace/MEMORY.md for curated context in heartbeat prompts
- Use oc://workspace/memory/ to browse recent session logs
```

This is a minor ergonomic improvement with no behavioral impact. No changes to prior HEARTBEAT.md template required otherwise.

### iMessage Urgency — 16 Days Silent, Escalated to Critical

The iMessage monitoring pause has now persisted 16 days. The **investigation step** (check connection type in AlphaClaw UI → iMessage section) is the single most impactful action that can be taken right now. It requires no configuration change and takes under 2 minutes.

**If the connection type is cloud proxy:** verify billing/auth status for the proxy service.
**If the connection type is local Mac bridge:** verify the Mac bridge process is running.

Only after confirming the connection is stable should monitoring be resumed.

---

## Updated Priority Order (2026-05-11 — iMessage Escalated)

1. **Investigate iMessage connection type in AlphaClaw UI** — under 2 minutes, zero risk, maximum urgency
...
5. **Create `workspace/MEMORY.md`** (Rec 3, updated with cloud proxy note + 53-day bootstrap stale note)
6. **Update OpenClaw to 2026.5.7** via AlphaClaw UI (exec-approvals seeded automatically)
7. **Run `openclaw doctor --fix`**
8. **Run `openclaw models auth list`**
9. **Reconnect Google Workspace** in AlphaClaw UI → regenerates 53-day-stale bootstrap TOOLS.md
10. **Populate `workspace/HEARTBEAT.md`** (Rec 1 + oc-path addendum + morning voice brief)
11. **Add Heather identity section to SOUL.md** (Rec 7)
12. **Add daily session log protocol to AGENTS.md** (Rec 8)
13. **Add learning loop to SOUL.md** (Rec 9)
14. **Enable streaming + compaction + memory-core with admin scope** (Rec 5/11)

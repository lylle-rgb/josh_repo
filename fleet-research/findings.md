...
**Agent:** AlphaClaw Apex Fleet Research Agent  
**Instance:** Josh — Discord bot Heather Schwartz (personal assistant: iMessage, email, calendar, contacts)  
**Previous findings:** See Day 19 Evening Scan above. All prior findings remain unresolved.  
**Scan type:** Platform news + new research findings.
...
Confirmed from web search as of this morning: 2026.5.7 is the latest stable. Web search returns 2026.5.6 as "last published 15 hours ago" — consistent with 2026.5.7 being a same-day follow-up hardening release. Both are captured in Day 19 Evening Scan.

**Josh version gap: 2026.3.22 → 2026.5.7 = 13 releases, 2.5+ months.**

---

### Gemini 3.1 Series Now Available on OpenRouter — Direct Upgrade Path for Josh
**New this morning.** The Gemini 3.1 model family is confirmed available on OpenRouter:

- **`google/gemini-3.1-flash-preview`**: Near-Pro reasoning, improved agentic reliability and tool orchestration vs. Gemini 3 Flash, broad quality improvements across reasoning, multimodal understanding, and reliability. **Natural successor to Josh's current primary model.**
- **`google/gemini-3.1-pro-preview`**: Enhanced software engineering and agentic stability, better long-horizon task reliability. Higher cost — relevant only for complex multi-step research work.
- **`google/gemini-3.1-flash-lite-preview`**: High-efficiency model, outperforms Gemini 2.5 Flash Lite. Budget tier — suitable as a low-cost heartbeat sub-task model override.

Josh's current primary (`google/gemini-3-flash-preview`) is the direct predecessor of 3.1 Flash. The upgrade is a one-line change to openclaw.json with the prior primary becoming a fallback — zero reliability regression.

**Risk:** Preview model — validate in a test session before committing as permanent primary.

---

### OpenTelemetry Observability — Now Fleet-Wide in 2026.5.x
Confirmed from research: OpenClaw 2026.5.x now emits OTel spans for model calls, token usage, tool loops, harness runs, exec processes, outbound delivery, context assembly, and memory pressure.

**Relevance for Josh:** Once HEARTBEAT.md is populated and memory-core installed, OTel traces will show which tool calls are slow (email checks, calendar lookups), actual token cost per heartbeat run, and memory-core query performance. This is monitoring infrastructure — no config needed to enable, available post-update.

---

### TTS Upgrade — ElevenLabs v3 + `/tts latest` Now Available
The 2026.5.x series includes a TTS overhaul: `/tts latest`, chat-scoped auto-TTS controls, per-agent/per-account voice overrides, and ElevenLabs v3 coverage.

**Relevance for Josh:** AGENTS.md already instructs Heather to use `sag` (ElevenLabs TTS) for voice storytelling — "Way more engaging than walls of text. Surprise people with funny voices." ElevenLabs v3 is a significant quality upgrade. The new `/tts latest` lets Josh or Heather switch TTS backend without a config restart.

**Action after update:** Populate `workspace/TOOLS.md` with a preferred voice name. Test with `/tts latest` in Discord.

---

## New Findings — Josh Instance (Day 20)

### 20. Gemini 3.1 Flash — Natural Primary Model Upgrade Now Available
**Risk: OPPORTUNITY**

The direct successor to Josh's current primary model is now on OpenRouter. Upgrade path maintains full reliability with the previous primary as fallback. Also corrects the retired `claude-3.5-haiku` fallback currently listed in openclaw.json.

**Suggested `agents.defaults.model` block in openclaw.json (after updating to 2026.5.7):**
```json
"model": {
...
Note: The third fallback (`claude-3.5-haiku`) in current openclaw.json is a **retired model** — replaced above with `claude-haiku-4-5-20251001`. Consider also adding `openrouter/google/gemini-3.1-flash-lite-preview` as a budget heartbeat model override.

**Risk:** Preview model — test in a session before committing. Watch for tool-call behavior changes in email/calendar tools.

---

## Implementation Status — Day 20

Version gap: **2026.3.22 → 2026.5.7 (13 releases, 2.5+ months)**. iMessage monitoring paused for the entire 20-day scan window. Bootstrap TOOLS.md Google contradiction now **50 days stale**. Zero implementations across all scan days.
...
| No daily memory files (sessions leave no trace) | HIGH | 6 | ⬜ Pending |
| MEMORY.md missing | MEDIUM | 20 | ⬜ Pending |
| SOUL.md never evolved (stock template) | MEDIUM | 6 | ⬜ Pending |
| No-emoji rule not in SOUL.md (contradiction) | MEDIUM | 6 | ⬜ Pending |
| Bootstrap TOOLS.md stale — Google contradiction (50 days) | MEDIUM | 20 | ⬜ Pending |
| inbox-state.json malformed JSON | MEDIUM | 20 | ⬜ Pending |
| TOOLS.md template only | MEDIUM | 20 | ⬜ Pending |
| Active Memory plugin not installed | MEDIUM | 20 | ⬜ Pending |
| Active Memory admin scope required (new in 5.7) | MEDIUM | 2 | ⬜ Noted |
| Retired claude-3.5-haiku fallback in openclaw.json | LOW | new | ⬜ Noted |
| Discord streaming off | LOW | 20 | ⬜ Pending |
| No compaction config | LOW | 20 | ⬜ Pending |
| Update before enabling heartbeats (sequencing) | INFO | 4 | ⬜ Noted |
| Gemini 3.1 Flash Preview available (upgrade path) | OPPORTUNITY | new | ⬜ Noted |

**Correct implementation order (unchanged from Day 19, target 2026.5.7):**
1. Fix `inbox-state.json` duplicate key (2 min)
2. Update OpenClaw to **2026.5.7** via AlphaClaw UI (5 min)
3. Run `openclaw doctor --fix` (1 min)
4. Run `openclaw models auth list` (1 min — verify Google auth state)
5. Reconnect Google Workspace in AlphaClaw UI → regenerates 50-day-stale bootstrap TOOLS.md (5 min)
6. Create `workspace/MEMORY.md` from soul-improvements.md template (15 min)
7. Populate `workspace/HEARTBEAT.md` — safe after 2026.5.5 heartbeat disconnect fix (15 min)
8. Fix no-emoji contradiction in SOUL.md (5 min)
9. Investigate iMessage pause (10 min)

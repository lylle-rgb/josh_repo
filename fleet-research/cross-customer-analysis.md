...
**Last updated:** 2026-06-29 morning (F66/F67/F68/F69 — Day 100 confirmed; day counts updated)
...
## Fleet Snapshot (June 29 Morning)
...
| **OpenClaw version** | 2026.3.22 (100 days old) | Unknown — last git sync March 2026 (~113+ days ago) |
| **Safe upgrade target** | **2026.6.10-stable** (Day 6 — green light) | Unknown — likely same staged path |
| **2026.6.11 status** | Beta only — confirmed still beta June 29 morning | Beta only — do not install |
...
| **Cron/heartbeat** | Not deployed (Day 15 null) | Unknown — critical for trading hours monitoring |
| **Google Workspace OAuth** | NOT connected **(🚨 Day 100 — TODAY June 29)** | N/A (trading bot) |
| **iMessage** | Paused since April 27 (Day 65) | N/A |
| **AlphaClaw version** | 0.9.18 (current — no 0.9.19 released as of June 29 morning) | Unknown |
| **Repo access** | ✅ `josh_repo` | ⛔ `noah--repo` (404) — correct repo is `Noahrepo2` |
| **Fleet coverage** | ✅ Active | ⛔ DARK — Day 20 |
...
Josh is confirmed on 2026.3.22 (100 days old as of June 29). Noah's version is unknown but git sync stopped in March — Noah is almost certainly on an older build as well.

**Current stable target: 2026.6.10** (Day 6 of stable as of June 29 — overnight clean signal, FULLY OPEN window).
**Skip 2026.6.8 and 2026.6.9** — both have confirmed critical regressions.
**2026.6.11 is still beta** (F67 — confirmed June 29 morning; do not install).
...
- Explicit compaction, dry-run message previews, session renaming, SSH preflight
...
Preview models retire on rolling schedule with minimal notice — `gemini-2.5-flash` wave confirmed (F52). Proactive migration to `google/gemini-3.5-flash` (GA stable) recommended for both customers.
...
- SkillSpector scanning is now standard on all new ClawHub installs — benefits future installs only.

### 4. Cron/Heartbeat Deployment (HIGH for Josh, critical-unknown for Noah)

- Josh: heartbeat cron not deployed — 15 days of null state. Must set `OPENCLAW_TIMEZONE=America/Los_Angeles` FIRST (via Envars tab), then bundle cron deployment with the 2026.6.10 upgrade. PR #96233 in 2026.6.10 directly fixes heartbeat prompt contribution.
...
**Critical (today — no upgrade needed):**
- **🚨 Google Workspace OAuth: Day 100 (TODAY June 29)** — Email and calendar inaccessible for 100 days. Fix: 5-minute OAuth click at https://5.78.142.81.sslip.io#general. Heather must surface at every main session until resolved. Next mandatory escalation: Day 110 (July 9).
- **OPENCLAW_TIMEZONE not set** — heartbeat cron will fire on UTC, misaligned with Josh's LA/PDT timezone. Set NOW via AlphaClaw Envars tab (`OPENCLAW_TIMEZONE=America/Los_Angeles`). Takes 30 seconds.
- **Model still on gemini-3-flash-preview** — migrate NOW via Browse tab to `google/gemini-3.5-flash`. No upgrade, no VPS, 5 minutes.
- **BRAVE_API_KEY missing** — Heather cannot search the web proactively. Set via AlphaClaw Envars tab.

**Requires upgrade to 2026.6.10:**
- iMessage monitoring paused since April 27 (Day 65) — auto-fix via SQLite migration at 2026.6.6 upgrade step
- Active Memory + Dreaming not enabled — both opt-in and available after upgrade past 2026.3.22
- Heartbeat cron not deployed — deploy in openclaw.json post-upgrade
- Discord streaming disabled — enable `partial` streaming post-upgrade
- Discord auto-thread titles — enable post-upgrade

**Upgrade checklist:**
1. Run `openclaw backup create` before first hop
2. Follow staged path: 2026.3.22 → 2026.5.27 → 2026.6.2 → 2026.6.5 → 2026.6.6 → 2026.6.10
3. After 2026.6.6 hop, verify iMessage inbox-state.json was cleared by SQLite migration
4. After 2026.6.10: enable Active Memory, Dreaming, Fast Talks; deploy heartbeat cron; upgrade fallback 2 to Haiku 4.5; enable Discord streaming

**Post-2026.6.10 planning:**
- Monitor 2026.6.11 beta for stable promotion — per-DM model overrides directly useful for Josh
...
- **Agent Mesh architecture** (emerging March 2026+): Research Agent → Risk Agent → Execution Agent — Noah may benefit from restructuring into this 3-stage pipeline
- ClawHub has 311+ finance/investing skills — `sec-filing-watcher` and `alpaca-trading` both confirmed operational
...
5. **Discord security lockdown** — restrict `allowFrom` to specific user IDs before any non-paper trading

---

## Fleet-Wide Recommendations (June 29 Morning)

### Immediate (today, no VPS needed)
1. **Josh:** Connect Google Workspace OAuth — **🚨 Day 100 TODAY** — https://5.78.142.81.sslip.io#general — 5 min
2. **Josh:** Set `OPENCLAW_TIMEZONE=America/Los_Angeles` via Envars tab
...
6. **Fleet admin:** Evaluate AlphaClaw Apex — unified Mac dashboard for Josh + Noah fleet management

### This Week (requires VPS)
7. **Josh:** Run `openclaw backup create` before upgrade
8. **Josh:** Execute staged upgrade to 2026.6.10 (SSH) — Day 6 of stable window, fully open
9. **Josh:** Bundle with upgrade: enable Active Memory + Dreaming, deploy heartbeat cron
10. **Noah (after scope fix):** Full workspace audit — version, skills, model config, Alpaca integration, Discord security
11. **Noah (after scope fix):** Staged upgrade to 2026.6.10
12. **Noah (after scope fix):** Evaluate Alpaca MCP Server v2 + `sec-filing-watcher` skill + Agent Mesh restructuring

### Ongoing
13. **Both:** Monitor 2026.6.11 for stable promotion — verify: `npm show openclaw@2026.6.11 version` after reaching 2026.6.10
14. **Both:** Check Google Gemini deprecation page monthly — retirement waves hit on schedule, no reprieves
15. **Both:** Run `openclaw skill list` after each upgrade — ClawHavoc risk is ongoing
16. **Both:** After upgrade, tighten Discord `allowFrom: ["*"]` to specific user IDs
...
|----------|------|--------|-----------------|----------|
| Josh | josh_repo | ✅ Active | June 29 morning (F66/F67/F68/F69) | ✅ Comprehensive |
